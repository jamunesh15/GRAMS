...
export default function TaskCompletionForm({ grievance, isOpen, onClose, onComplete }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!completionImage) {
      toast.error('Please select an after-task image');
      return;
    }
    ...
    if (allocatedBudget > 0 && isOverBudget) {
      const confirm = window.confirm(
        `Warning: You have exceeded the allocated budget by ₹${Math.abs(remainingBudget).toLocaleString()}. Do you want to proceed?`
      );
      if (!confirm) return;
    }

    setSubmitting(true);
    const loadingToast = toast.loading('Uploading and completing task...');

    try {
      // Create FormData with all files and data
      const formDataToSend = new FormData();
      formDataToSend.append('grievanceId', grievance._id);
      formDataToSend.append('completionNotes', formData.completionNotes || '');
      formDataToSend.append('daysToComplete', formData.daysToComplete);
      formDataToSend.append('expenses', JSON.stringify(validExpenses));
      formDataToSend.append('completionImage', completionImage);
      
      // Add all bill images
      billImages.forEach((file) => {
        formDataToSend.append('billImages', file);
      });
                  ...
                  <div className="space-y-2">
                    ...
                    {allocatedBudget > 0 && (
                      <>
                        ...
                        {isOverBudget && (
                          <div className="mt-2 p-3 bg-red-100 border border-red-300 rounded-lg flex items-start gap-2">
                            <svg className="w-5 h-5 text-red-600 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                            </svg>
                            <p className="text-sm text-red-800 font-medium">
                              Warning: You have exceeded the allocated budget. Please provide justification in notes.
                            </p>
                          </div>
                        )}
                      </>
